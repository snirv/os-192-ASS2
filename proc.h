...
struct cpu {
  ...
  struct thread * thread;       // The thread running on this cpu or null   // 2.1 add
};
...
//  char *kstack;                // Bottom of kernel stack for this process //2.1 move to thread
  enum procstate state;        // Process state
  int pid;                     // Process ID
  struct proc *parent;         // Parent process
//  struct trapframe *tf;        // Trap frame for current syscall //2.1 move to thread
//  struct context *context;     // swtch() here to run process //2.1 move to thread
